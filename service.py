...
def check_resync(diff_ms_allowed, jump_ahead_time, manual_adjust_ms, master_ip, master_port):
    xbmc.log("Init resync check", level=xbmc.LOGDEBUG)
    if xbmc.Player().isPlaying():
        endpoint = master_ip + ':' + '%d' % (master_port)
        url = '/playback/time'
        # xbmc.log("Connecting to " + endpoint + url, level=xbmc.LOGDEBUG)
        ...
        while (master_time >= 0) and (i < max_resync_loops) and (master_own_time_diff >= (diff_ms_allowed / 1000.0)):
            xbmc.log("sta %f master %f vs slave %f - %f >= %f?" % (i, own_time, master_time, master_own_time_diff, (diff_ms_allowed / 1000.0)), level=xbmc.LOGDEBUG)
            # resync
            ...
            xbmc.log("mid1 %f own_time %f vs master_time %f - wait_for_pause1 %f" % (i, own_time, master_time, wait_for_pause1), level=xbmc.LOGDEBUG)
            ...
            own_time = xbmc.Player().getTime()
            master_own_time_diff = abs(own_time - master_time)
            xbmc.log("fin %f own_time %f vs master_time %f - %f >= %f? - rounds %f" % (i, own_time, master_time, master_own_time_diff, (diff_ms_allowed / 1000.0), round), level=xbmc.LOGDEBUG)
            # increase accuracy with the first rounds
            # if (i < 1) and (diff_ms_allowed >= 65.0):
            #    diff_ms_allowed = 65
            i += 1
        if master_time < 0:
            xbmc.log("Cannot connect to " + endpoint, level=xbmc.LOGDEBUG)
        conn.close()
    else:
        xbmc.log("Kodi is not playing", level=xbmc.LOGDEBUG)
...
if __name__ == '__main__':
    ...
    xbmc.log('Starts play-sync the REST server at own_port %d' % own_port,level=xbmc.LOGDEBUG)
    http_server = StoppableHTTPServer(('', own_port), MyRequestHandler)
    thread = threading.Thread(None, http_server.run)
    thread.start()
    xbmc.log('REST server at own_port %d started' % own_port,level=xbmc.LOGDEBUG)
